...
.lang-btn{
  ...
  color:var(--muted2);font-size:12px;padding:5px 12px;
...
.connect-btn{
  ...
  padding:8px 18px;border-radius:var(--r2);border:none;
  transition:all 0.2s;
  box-shadow:0 0 16px rgba(34,184,230,0.2);
}
...
.tab{
  flex:1;padding:9px 0;border:none;border-radius:var(--r2);
...
.form-input{
  ...
  border-radius:var(--r2);padding:10px 14px;color:var(--text);font-size:14px;
...
.token-tab{
  flex:1;padding:8px;border:1px solid var(--border);border-radius:var(--r2);
  background:transparent;color:var(--muted);font-size:13px;font-weight:500;
  transition:all 0.2s;
}
...
.denom-btn{
  ...
  background:transparent;color:var(--muted2);font-size:12px;font-family:var(--mono);
  transition:all 0.2s;
}
...
.toggle-slider{
  position:absolute;inset:0;background:var(--surface2);border:1px solid var(--border2);
  border-radius:20px;cursor:pointer;transition:all 0.3s;
}
.toggle-slider::before{
  content:'';position:absolute;width:16px;height:16px;border-radius:50%;
  background:var(--muted);left:2px;top:2px;transition:all 0.3s;
}
...
.send-btn{
  ...
  background:var(--accent);color:var(--bg);font-size:14px;font-weight:600;
  transition:all 0.2s;margin-top:8px;
  box-shadow:0 0 20px rgba(34,184,230,0.2);
}
...
.history-token{
  ...
  border:1px solid rgba(34,184,230,0.2);display:flex;align-items:center;
  justify-content:center;font-size:11px;font-weight:600;color:var(--accent);flex-shrink:0;
}
.history-info{flex:1}
...
.key-btn{
  flex:1;padding:7px;border:1px solid var(--border2);border-radius:var(--r2);
  background:transparent;color:var(--muted2);font-size:12px;transition:all 0.2s;
}
...
.paylink-box{
  background:var(--surface2);border:1px solid rgba(34,184,230,0.2);
...
.paylink-copy{
  width:100%;margin-top:10px;padding:8px;border:1px solid var(--accent);
  border-radius:var(--r2);background:var(--accent-dim);color:var(--accent);
...
/* SCAN */
.scan-result{
  background:var(--surface2);border:1px solid rgba(52,211,153,0.2);
  border-radius:var(--r);padding:20px;margin-top:16px;
}
...
.withdraw-btn{
  width:100%;padding:9px;border:1px solid var(--accent);border-radius:var(--r2);
...
.modal-btn{flex:1;padding:10px;border-radius:var(--r2);font-size:13px;font-weight:600;border:none;transition:all 0.2s}
...
/* NOTIF BANNER */
.notif-banner{
  background:rgba(34,184,230,0.08);border:1px solid rgba(34,184,230,0.2);
  border-radius:var(--r2);padding:10px 16px;margin-bottom:20px;
  display:flex;align-items:center;justify-content:space-between;gap:12px;
  font-size:13px;color:var(--muted2);
}
.notif-btn{
  padding:5px 12px;border:1px solid var(--accent);border-radius:20px;
  background:transparent;color:var(--accent);font-size:12px;white-space:nowrap;
  transition:all 0.2s;
}
...
.how-num{
  ...
  border:1px solid rgba(34,184,230,0.3);display:flex;align-items:center;
  justify-content:center;font-family:var(--mono);font-size:11px;color:var(--accent);flex-shrink:0;margin-top:1px;
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (match) {
      setRecipient(decodeURIComponent(match[1]));
      setTab("send");
    }
  }, []);
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (data.status === "completo") {
          ...
          setPipelineId(null);
          sendNotification(t.send, "Transação concluída com sucesso!");
          setStatusType("success");
          setStatus("✓ " + (lang === "pt" ? "Transação concluída!" : "Transaction complete!"));
        }
      } catch {}
    }, 5000);
  ...
  async function send() {
    ...
    if (!val || val <= 0) return showStatus("Informe o valor.", "error");
    if (!recipient.trim()) return showStatus("Informe o destinatário.", "error");
    ...
    try {
      // Get disposable entry address from backend
      const entryRes = await fetch(
        `${BACKEND_URL}/entrada?timelocked=${useTimelocked}&token=${token}`
      );
      const { endereco: entryAddress } = await entryRes.json();
      if (!entryAddress) throw new Error("Backend não retornou endereço de entrada.");
      ...
      if (token === "ETH") {
        ...
        await tx.wait();
        saveHistory({ hash: tx.hash, token, amount: val, to: recipient, ts: Date.now(), status: "pending" });
        // Poll for pipeline
        await pollPipeline(entryAddress);
      } else {
        const tc = new ethers.Contract(TOKENS[token].address, ERC20_ABI, signer);
        const allow = await tc.allowance(account, entryAddress);
        if (allow < valBig) {
          const a = await tc.approve(entryAddress, valBig);
          await a.wait();
        }
        // For tokens, send to entry address — backend handles the rest
        const erc20Full = new ethers.Contract(
          TOKENS[token].address,
          ["function transfer(address to, uint256 amount) external returns (bool)"],
          signer
        );
        const tx = await erc20Full.transfer(entryAddress, valBig);
        await tx.wait();
        saveHistory({ hash: tx.hash, token, amount: val, to: recipient, ts: Date.now(), status: "pending" });
        await pollPipeline(entryAddress);
      }
  ...
  async function pollPipeline(entryAddress) {
    try {
      const aguardarRes = await fetch(`${BACKEND_URL}/aguardar/${entryAddress}`);
      const data = await aguardarRes.json();
  ...
  async function scan() {
    ...
    try {
      ...
      for (const ev of events) {
        const { ephemeralPubKey, stealthAddress, token: tokenAddr, amount, viewTag, timelocked, unlockAt } = ev.args;
        const result = tryDecryptDeposit(ephemeralPubKey, stealthAddress, Number(viewTag), spendingKey, viewingKey);
        if (result) {
          const tokenSym = Object.keys(TOKENS).find(k => TOKENS[k].address.toLowerCase() === tokenAddr.toLowerCase()) || "?";
          const decimals = TOKENS[tokenSym]?.decimals || 18;
          found.push({
  ...
  async function doWithdraw(item) {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        showStatus(lang === "pt" ? "Saque realizado com sucesso!" : "Withdrawal successful!", "success");
  ...
  async function handleImport() {
    if (!modalPwd || !importData) return;
    try {
      const { spendingKey: sk, viewingKey: vk } = await decryptKeys(importData.trim(), modalPwd);
      setSpendingKey(sk); setViewingKey(vk);
      localStorage.setItem("sf_spending", sk);
      localStorage.setItem("sf_viewing", vk);
      buildMetaAddress(sk, vk);
      setShowImport(false); setModalPwd(""); setImportData("");
  ...
  const pipelineSteps = [
    ...
    { key: "splitting",  label: lang === "pt" ? "Split em denominações" : "Splitting into denominations" },
  ...
  function getStepStatus(stepKey) {
    ...
    const currentIdx = order.indexOf(pipelineData.status);
    ...
    <>
      ...
      <div className="app">
        ...
        <main className="main">
          ...
          {/* STATUS */}
          {status && (
            <div className={`alert alert-${statusType} fade-up`}>{status}</div>
          )}

          {/* TABS */}
          <div className="tabs">
            {["send","receive","scan","withdraw"].map(tabKey => (
              <button
                key={tabKey}
                className={`tab ${tab === tabKey ? "active" : ""}`}
                onClick={() => { setTab(tabKey); setStatus(""); }}
              >
                {t[tabKey]}
              </button>
            ))}
                ...
                <div className="card">
                  ...
                  <div className="toggle-row">
                    ...
                    <label className="toggle">
                      <input type="checkbox" checked={useFixedDenom} onChange={e => { setUseFixedDenom(e.target.checked); setSelectedDenom(null); setAmount(""); }} />
                    ...
                    <div className="form-group" style={{marginTop:14}}>
                      ...
                      <div className="denom-grid">
                        {DENOMS[token].map(d => (
                          <button
                            key={d}
                            className={`denom-btn ${selectedDenom === d ? "active" : ""}`}
                    ...
                    history.map((h, i) => (
                      <div key={i} className="history-item">
                        ...
                        <div className="history-info">
                          ...
                          <div className="history-to">{h.to?.slice(0,20)}...</div>
            ...
            <div className="grid fade-up">
                ...
                {!spendingKey ? (
                  <>
                    <p style={{color:"var(--muted)",fontSize:13,marginBottom:20}}>
                      {lang === "pt"
                        ? "Gere suas chaves stealth para receber pagamentos de forma privada."
                        : "Generate your stealth keys to receive payments privately."}
                    </p>
                  ...
                  <>
                    ...
                    <div className="key-box">
                      ...
                      <div className="key-value" style={{filter:"blur(4px)",userSelect:"none"}} onMouseEnter={e=>e.target.style.filter="none"} onMouseLeave={e=>e.target.style.filter="blur(4px)"}>
                        {spendingKey}
                      </div>
                    </div>
              ...
              <div className="card">
                ...
                <p style={{color:"var(--muted)",fontSize:13,marginBottom:16}}>
                  {lang === "pt" ? "Já tem um backup? Importe aqui." : "Have a backup? Import it here."}
                </p>
                <div className="form-group">
                      ...
                      <div key={i} className="scan-item">
                        ...
                        {r.timelocked && r.unlockAt > Date.now()/1000 && (
                          <div className="scan-locked">
                            🔒 {t.locked}: {new Date(r.unlockAt * 1000).toLocaleString()}
                          </div>
                        )}
                        <a href={`https://basescan.org/tx/${r.txHash}`} target="_blank" rel="noreferrer" style={{fontSize:11,color:"var(--accent)"}}>
                          Basescan ↗
                        </a>
                      </div>
                    ))}
              ...
              <div className="card">
                ...
                <div className="how-list">
                  <div className="how-item">
                    ...
                    <div className="how-text">
                      <h4>{lang === "pt" ? "Busca eventos on-chain" : "Fetches on-chain events"}</h4>
                      <p>{lang === "pt" ? "Busca os últimos 50.000 blocos no contrato SilentFlow." : "Scans the last 50,000 blocks on the SilentFlow contract."}</p>
                    </div>
                  </div>
                  <div className="how-item">
                    <div className="how-num">2</div>
                    <div className="how-text">
                      <h4>{lang === "pt" ? "Testa cada depósito" : "Tests each deposit"}</h4>
                      <p>{lang === "pt" ? "Usa suas chaves privadas localmente para tentar descriptografar." : "Uses your private keys locally to try decrypting each event."}</p>
                    </div>
                  </div>
                  <div className="how-item">
                    <div className="how-num">3</div>
                    <div className="how-text">
                      <h4>{lang === "pt" ? "100% local" : "100% local"}</h4>
                      <p>{lang === "pt" ? "Suas chaves nunca saem do browser. Zero exposição." : "Your keys never leave the browser. Zero exposure."}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="card">
                ...
                {scanResults.length === 0 ? (
                  <div style={{textAlign:"center",padding:"40px 20px",color:"var(--muted)",fontSize:13}}>
                    {t.noDeposits}
                    <br />
                    <button className="key-btn" style={{marginTop:12,width:"auto",padding:"8px 20px"}} onClick={() => setTab("scan")}>
                      → {t.scan}
                    </button>
                  </div>
                ) : (
        ...
        {showExport && (
          <div className="modal-overlay" onClick={() => setShowExport(false)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="form-group">
                <div className="form-label">{t.password}</div>
                <input className="form-input" type="password" placeholder="••••••••" value={modalPwd} onChange={e => setModalPwd(e.target.value)} />
              </div>
        ...
        {showImport && (
          <div className="modal-overlay" onClick={() => setShowImport(false)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="form-group">
                <textarea className="form-input" rows={3} placeholder="Conteúdo do .enc" value={importData} onChange={e => setImportData(e.target.value)} />
              </div>
              <div className="form-group">
                <input className="form-input" type="password" placeholder="Senha" value={modalPwd} onChange={e => setModalPwd(e.target.value)} />
              </div>
              <div className="modal-actions">
                <button className="modal-btn secondary" onClick={() => setShowImport(false)}>{t.cancel}</button>
                <button className="modal-btn primary" onClick={handleImport}>{t.confirm}</button>
              </div>
            </div>
          </div>
        )}
