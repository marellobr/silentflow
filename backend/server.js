...
// Valor minimo configuravel por env (default: 0.005 ETH para testnet)
...
// masterWallet: financia gas das wallets efemeras e coleta taxas
const masterWallet = new ethers.Wallet(process.env.CARTEIRA_PRIVADA, provider);

// Mapa de entradas descartaveis: endereco ? { wallet, token, stealthAddress, ephemeralPubKey, viewTag, criadoEm }
const entradasPendentes = new Map();

// Fila de pipelines em memoria
const fila = new Map();
...
async function executarPipelineETH(txId, valorBruto, stealthAddress, ephemeralPubKey, viewTag) {
  ...
  console.log(`  ? Taxa coletada: ${ethers.formatEther(taxa)} ETH | Liquido: ${ethers.formatEther(valorLiquido)} ETH`);
  ...
  const promessas = partes.map(async (valorParte, i) => {
    ...
    try {
      ...
      // Delay final + deposito no contrato
      const delayFinal = delayAleatorio(30, 120);
      console.log(`  ? Split ${i+1} deposito final: aguardando ${Math.round(delayFinal/1000)}s...`);
      ...
      if (!depositHash) throw new Error(`Deposito final falhou no split ${i}`);
...
async function executarPipelineToken(txId, tokenAddress, valorBruto, stealthAddress, ephemeralPubKey, viewTag) {
  ...
  const { valorLiquido, taxa } = descontarTaxa(valorBruto);
  console.log(`  ? Taxa coletada: ${taxa.toString()} units | Liquido: ${valorLiquido.toString()} units`);
  ...
  // Token: hops em serie pois cada um precisa financiar gas da master
  const depositHashes = [];
  for (let i = 0; i < partes.length; i++) {
...
// ??? Monitoramento de entradas descartaveis ???????????????????????????????????
...
async function monitorarEntradas() {
  for (const [endereco, entrada] of entradasPendentes.entries()) {
    // Expira apos 30 minutos sem uso
    ...
    try {
      ...
      if (valorRecebido < minimo) {
        console.log(`??  Entrada ${endereco.slice(0,10)}... recebeu ${valorRecebido} mas minimo e ${minimo} ? aguardando mais`);
        continue;
      }
      ...
      // Para ETH: move ETH da entrada para a master, depois dispara pipeline
      // Para token: tokens ja estao na entrada, dispara pipeline direto
      ...
      // Notifica frontend via polling
      entrada.resolveId && entrada.resolveId(id);

      if (entrada.token === "ETH") {
        // Move ETH da entrada para a master (consolidacao), depois pipeline
        const gasPrice = await getGasPrice();
        const gasCost  = gasPrice * 21000n;
...
// GET /entrada
// Gera uma wallet descartavel de entrada para o usuario enviar fundos
...
app.get("/aguardar/:endereco", (req, res) => {
  ...
  // Ainda pendente -- nao processou
  if (entradasPendentes.has(endereco)) {
    return res.json({ recebido: false });
  }

  // Nao esta mais pendente -- busca pipeline na fila pelo entradaAddress
  ...
  // Processou mas nao encontrou na fila ainda (race condition) -- aguarda
  return res.json({ recebido: false });
});
...
// GET /minimos ? retorna valores minimos para o frontend mostrar
...
app.listen(PORT, () => {
  console.log(`? SilentFlow backend v4 (entrada descartavel + taxa + minimos) ? porta ${PORT}`);
  console.log(`? Master wallet: ${masterWallet.address}`);
  console.log(`? Taxa: 0.2% | Minimos: ${ethers.formatEther(MIN_ETH)} ETH / ${ethers.formatUnits(MIN_USDC,6)} USDC / ${ethers.formatUnits(MIN_USDT,6)} USDT`);
});
